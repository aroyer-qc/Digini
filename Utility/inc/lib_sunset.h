...
#define SUNSET_OFFICIAL             90.833          // Standard sun angle for sunset
#define SUNSET_NAUTICAL             102.0           // Nautical sun angle for sunset
#define SUNSET_CIVIL                96.0            // Civil sun angle for sunset
#define SUNSET_ASTONOMICAL          108.0           // Astronomical sun angle for sunset
...
class SunSet
{
    public:
        ...
        double                  SetCurrentDate                      (int, int, int);            // use digini methos for set time, or time lib
        double                  CalcNauticalSunrise                 (void) const;        // not usefull
        double                  CalcNauticalSunset                  (void) const;         // not usefull
        double                  CalcCivilSunrise                    (void) const;
        double                  CalcCivilSunset                     (void) const;
        double                  CalcAstronomicalSunrise             (void) const;
        double                  CalcAstronomicalSunset              (void) const;
        double                  CalcCustomSunrise                   (double) const;
        double                  CalcCustomSunset                    (double) const;
    [[deprecated("UTC specific calls may not be supported in the future")]] double calcSunriseUTC(void);
    [[deprecated("UTC specific calls may not be supported in the future")]] double calcSunsetUTC(void);
        double                  CalcSunrise                         (void) const;
        double                  CalcSunset                          (void) const;
        int                     MoonPhase                           (int) const;
        int                     MoonPhase                           (void) const;
